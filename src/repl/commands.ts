...
/**
 * The in-session help, rewritten in S21.5 around what a user needs to know, in the order they
 * need it.
 *
 * The old version was one undivided wall of 21 commands with the two most consequential — `/plan
 * approve` and `/accept` — buried among status views, three of the four sigils documented and none
 * of the aliases. The shape now mirrors the real surface: most work is conversation, decisions are
 * ASKED at the moment they arise, and these commands are the escape hatches and inspection tools
 * for driving it yourself.
 */
export const HELP = [
  'Most of the time, just talk. Agent CLI asks when a decision is actually needed — approving a',
  'plan, finishing the session, closing an escalation — so there is no lifecycle to memorize.',
  '(Those prompts need a real terminal; piped and --no-input runs use the commands below.)',
  '',
  'ask a specialist:',
  '  @plan <request>       investigate and plan first; nothing mutating runs until you approve',
  '  @review [focus]       inspect the code for bugs, weak points and regressions (advice — blocks nothing)',
  '  @search <question>    one bounded web lookup, answered from snippets',
  '  @research <question>  delegate a research subagent that answers from sources, with provenance',
  '',
  'see and undo:',
  '  /diff                 what this session changed (unified diff vs the pre-images)',
  ...
  '                        commit session-attributed changes after a preview + confirmation',
  '  /checkpoint [label | list | restore <n>]',
  '                        capture the workspace to a hidden git ref; restore returns to one as a',
  '                        single undoable batch (your git history is never touched)',
  '',
  'drive it yourself:',
  '  /plan [show | approve | discard]   the plan document and its approval gate',
  '  /accept [confirm]                  the completion boundary; confirm records a PARTIAL',
  '                                     acceptance when work is unfinished',
  '  /repair [dismiss <n> <reason>]     the repair ledger; a dismissal is YOUR decision, kept as an',
  '                                     acceptance caveat and never as a resolution',
  '  /provider [name [model]] · /model [id]',
  '                                     list or switch, between turns (credentials are env-only)',
  '  /grants [revoke <id>]              durable "always allow" records; a revoke applies next session',
  '  /init                              create your global AGENT.md (and a starter project one)',
  '  /quit                              end the session (/exit and Ctrl+D also work)',
  '',
  'also available, mostly for inspection: /checks /review /research /remote /preview /tasks /map',
