...
export function buildSystemPrompt(workspaceRoot: string, map: WorkspaceMap, sandbox?: EnforcementFacts, git?: GitFacts, memory?: SystemPromptMemory): string {
  ...
  return [
    ...
    '- Review is a STRUCTURAL GATE, not a suggestion: a plan with executor tasks requires ONE recorded adversarial review round after integration, and /accept refuses without it (a plan can waive it only explicitly, with a reason the user approves). After the implementation passes its checks, run a single delegate_task call with 2–3 reviewer tasks, each a DIFFERENT lens (e.g. correctness, security/policy, test coverage), each given a diff scoped to its lens via context. Reviewers RECORD findings through report_finding — recorded findings are the ONLY gate input; their prose is narration. Then verify every critical/high finding against the actual code YOURSELF and triage it with the review tool: refute with evidence (recorded verbatim as your unverified claim), address with cited fix refs and re-run the checks that prove it, or accept (medium/low only — a recorded limitation). Never a bigger panel, never a re-review per finding, at most 2 rounds; post-review fixes re-run the completion checks, not the review. Open critical/high findings block acceptance until triaged — /accept confirm is the USER\'s override, never yours.',
