...
export interface SystemPromptMemory {
  ...
  /** S21: durable research findings (harness-managed; perishable, dated per entry). */
  researchText?: string;
  researchTruncated?: boolean;
  crashNote?: string;
}
...
export function buildSystemPrompt(
  ...
  /**
   * Session 19: whether the research capability is actually available (a credential is configured
   * and the tools were registered). The paragraph is conditional for the same reason the
   * retrieval line is: a model told about a tool its registry does not contain will reach for it,
  ...
  return [
    ...
    ...(git?.isRepo
      ? [
          ...
          ...(gitTools === true
            ? [
                '- Before risky or hard-to-reverse work — a large refactor, a migration, a sweeping rewrite — take a recovery point with `git_checkpoint` and say you did. It writes a commit object plus one hidden ref: it is on no branch, and their branches, tags, index and HEAD are untouched (`git log --all` does show it), and they can walk the workspace back to it with /checkpoint restore. It refuses if the capture would sweep in secret-named files that .gitignore does not already exclude — fix that by gitignoring them, never by working around it. The session allowance is small, so take points at real boundaries, not per edit.',
              ]
            : []),
          '- You have NO way to commit on your own initiative, and this is deliberate: committing is the user\'s decision, and the harness offers it to them at the acceptance boundary (they can also type /commit at any time). Never stage, commit, or otherwise modify version-control state the user can see (git add/commit/branch/checkout/restore/stash/reset/…) — not through run_command, not by any other route — unless the user explicitly asks you to in this session, in which case they still approve the exact command. Describe what you changed and why; let them decide what becomes history.' +
            (remote !== undefined
...
/**
 * The system prompt for the `@review` INSPECTOR subagent (Session 21.5).
 *
 * The reviewer's brief is adversarial and change-scoped: pick apart THIS diff through THIS lens.
 * The inspector's is diagnostic and codebase-scoped: a user has stopped mid-session and asked
 * "look at this and tell me what's wrong". Those want different instructions, which is the second
 * reason (after the gate coupling) that this is its own role.
 *
 * The most important line here is the last one: it can change nothing, so its job is to make the
 * PARENT able to act. An observation with no path, no evidence and no suggested next step is a
 * feeling, and a feeling costs the user a round trip.
 */
export function buildInspectorSystemPrompt(
  workspaceRoot: string,
  map: WorkspaceMap,
  sandbox?: EnforcementFacts,
  git?: GitFacts,
  ...
  return buildReadOnlySubagentPrompt(
    'You are a read-only INSPECTION subagent of Agent CLI, invoked by the user with @review. You examine the CURRENT state of this codebase — not a diff — for real defects: bugs that are wrong now, regressions a plausible change would cause, architectural problems that will keep costing, fragile spots, and concrete leads on failures the user is already chasing.',
    [
      '- Go where the risk is, not where reading is easy. Prefer the code the assigned focus names; then the seams around it — error paths, boundary conditions, concurrency, resource cleanup, state that outlives a request, and anything the tests do not touch. Read the REAL file before saying anything about it.',
      '- RECORD each item through the report_observation tool AS YOU CONFIRM IT: kind (bug / regression-risk / architecture / weak-spot / debug-lead), severity, affected paths, the evidence you ACTUALLY inspected (file:line and what the code does), the concrete scenario (inputs/state → wrong outcome, or why this bites later), your confidence, and — when you have one — what the parent should do to confirm or fix it. RECORDED observations are the only thing that reaches the parent as evidence; prose is narration.',
      '- Budget: 10 observations. Consolidate variants of one problem. An honest ZERO is a real answer and a useful one — say what you inspected and why you believe it is sound. Never invent a finding to look thorough.',
      '- Your observations do NOT block acceptance and do NOT count as an adversarial review round. Severity here means "how much this matters", not "how hard this stops the session".',
      '- You can change NOTHING: no writes, no commands that mutate. Your value is that the parent can act on what you hand back, so every observation must be specific enough to act on without re-deriving it.',
      '- Your FINAL message is a short prose summary for the main agent: what you inspected and how deeply, the observations you recorded (titles only — the records carry the detail), what you deliberately did not examine, and one line of overall confidence.',
    ].join('\n'),
    { workspaceRoot, map, sandbox, git, agentMd, retrieve },
  );
}
...
export function buildResearcherSystemPrompt(
  ...
  return buildReadOnlySubagentPrompt(
    ...
    [
      ...
      '- RECORD each finding through record_source AS YOU CONFIRM IT — before you read the next page, not at the end. RECORDED findings are what reaches the main agent as structured evidence; a claim that exists only in your prose is narration, and if your wall clock runs out mid-reading, everything you have not recorded is lost with the task. Give each one the claim as a falsifiable sentence, the source URLs it rests on, whether the sources corroborate or disagree, your confidence, and why it matters to the delegated task. Record findings, not summaries of the web.',
      '- YOUR WALL CLOCK IS REAL and every provider call spends it — a page read can take half a minute. Budget for finishing: reach an answer with what you have rather than reading one more page, and never leave your findings unrecorded while you keep searching.',
...
function memorySections(memory?: SystemPromptMemory): string[] {
  ...
  const hasLessons = memory.lessonsText !== undefined && memory.lessonsText.length > 0;
  const hasResearch = memory.researchText !== undefined && memory.researchText.length > 0;
  if (hasJournal || hasCodebase || hasLessons || hasResearch) {
    lines.push(
      '',
    ...
    if (hasLessons) {
      lines.push(
        '--- LESSONS.md (durable project lessons, newest first — pitfalls and failure patterns recorded by real sessions; check the relevant ones before repeating their mistakes) begin ---',
        neutralizeHarnessDelimiters(memory.lessonsText!.trimEnd()),
        ...(memory.lessonsTruncated === true ? [TRUNCATION_MARKER] : []),
        '--- LESSONS.md end ---',
      );
    }
    if (hasResearch) {
      lines.push(
        '--- RESEARCH.md (recorded research findings with sources — PERISHABLE: each entry carries its retrieval date, and the web may have moved since; re-verify anything load-bearing) begin ---',
        neutralizeHarnessDelimiters(memory.researchText!.trimEnd()),
        ...(memory.researchTruncated === true ? [TRUNCATION_MARKER] : []),
        '--- RESEARCH.md end ---',
      );
    }
  }
