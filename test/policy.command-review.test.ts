...
const ADVERSARIAL: { cmd: string; why: string }[] = [
  ...
  // S22.5: mutating/network forms hiding behind "read-only" subcommand NAMES. Every one of
  // these auto-allowed before the per-subcommand argument rules landed.
  { cmd: 'git tag v1.0.0', why: 'tag CREATION via a read-only subcommand name' },
  { cmd: 'git tag -d v1.0.0', why: 'tag deletion' },
  { cmd: 'git tag -f v1.0.0 HEAD', why: 'tag overwrite' },
  { cmd: 'git branch feature', why: 'branch CREATION' },
  { cmd: 'git branch -D main', why: 'branch deletion' },
  { cmd: 'git branch -m old new', why: 'branch rename' },
  { cmd: 'git remote add origin ./bare', why: 'remote add rewrites .git/config (relative path evades the path guard)' },
  { cmd: 'git remote set-url origin ./elsewhere', why: 'remote set-url rewrites .git/config' },
  { cmd: 'git remote remove origin', why: 'remote removal' },
  { cmd: 'git remote update', why: 'network fetch via remote update (the sandbox does not confine network)' },
  { cmd: 'git remote show origin', why: 'remote show contacts the network' },
  { cmd: 'git remote prune origin', why: 'prune deletes remote-tracking refs and contacts the network' },
  { cmd: 'git symbolic-ref HEAD refs/heads/evil', why: 'symbolic-ref WRITE form rewrites HEAD' },
  { cmd: 'git symbolic-ref --delete HEAD', why: 'symbolic-ref deletion' },
  { cmd: 'git reflog expire --all', why: 'reflog expire rewrites the reflog' },
  { cmd: 'git reflog delete xyz', why: 'reflog delete' },
];
