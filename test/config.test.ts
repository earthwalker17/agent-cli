...
describe('config loading and merging', () => {
  it('no files → empty rules, no prefs, no sources', () => {
    ...
    expect(c).toEqual({ rules: { protectedPaths: [], secretPatterns: [], envExcludePatterns: [] }, sources: [] });
  });
...
describe('narrowing effects on policy', () => {
  const rules = (r: Partial<PolicyRules>): PolicyRules => ({ protectedPaths: [], secretPatterns: [], envExcludePatterns: [], ...r });
