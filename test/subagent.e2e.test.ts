...
const DELEGATE_CALL = {
  name: 'delegate_task',
  input: { role: 'explorer', task: 'Survey the workspace and report what a.txt contains.' },
};
...
describe('delegate_task end to end', () => {
  ...
  it('caps delegation at TASKS_PER_SESSION; the refusal spawns nothing', async () => {
    ...
    for (let i = 0; i < TASKS_PER_SESSION; i++) {
      const r = await tool.execute({ role: 'explorer', task: `t${i}` }, ctx);
      expect(r.ok, `task ${i}`).toBe(true);
    }
    const before = fs.readdirSync(layout.sessionsDir).filter((f) => f.endsWith('.jsonl')).length;
    const ninth = await tool.execute({ role: 'explorer', task: 'one too many' }, ctx);
    expect(ninth.ok).toBe(false);
    expect(ninth.error).toContain('task budget exhausted');
