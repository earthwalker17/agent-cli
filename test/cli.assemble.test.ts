...
function makeDeps(overrides: Partial<AssembleDeps> = {}): AssembleDeps {
  ...
  return {
    ...
    config: { rules: { protectedPaths: [], secretPatterns: [], envExcludePatterns: [], researchBlockedDomains: [], remoteBlockedHosts: [] }, sources: [] },
    ctx: {
      ws,
...
describe('assembleSession', () => {
  ...
  it('assigns the log observer before the post-start records so a live view misses nothing', async () => {
    ...
    expect(seen).toEqual(['trust.verified', 'config.loaded', 'sandbox.status', 'git.context', 'remote.context', 'workspace.mapped', 'memory.loaded']);
    endSession(session, 'completed');
  });
